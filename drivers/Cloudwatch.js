...
const WinstonCloudwatchDriver = require('winston-cloudwatch')

/**
 * Winston console transport driver for @ref('Logger')
 * All the logs will be sent to Cloudwatch.
 *
...
class WinstonCloudwatch {
  ...
  setConfig (config) {
    ...
    this.logger = Winston.createLogger({ transports: [new WinstonCloudwatchDriver(this.config)] })
